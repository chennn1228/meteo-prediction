...
**云量合同关闭：**Open-Meteo 官方确认 Previous Runs 历史档案只保存 total
`cloud_cover`，不保存 `cloud_cover_low/mid/high`。历史主线据此正式采用 15 个变量；
不以 ERA5 回填、不插补、不把仅覆盖 2026-04 左右以后的 Single Runs 拼接进主研究。
该删除基于上游产品归档事实，不是为了让训练通过。

**raw 分布审计：**20 站 × 31 月 × 3 lead 共 1,357,920 行已直接从旧原始 JSON
重算，未训练、未联网。`kt_raw` 不截断；旧 `kt=1.5`、`kni=1.5`、
`diffuse_fraction=1` 条带确认由源码硬截断制造，相关旧图降为 provisional/legacy。
太阳几何与晴空辐射已改为 preceding-hour 区间中点/区间平均。区间中点太阳高度
>15°时 raw GHI 零值仅 3 行（2 个事件，约 0.00054%），同时 Himawari GHI
>300 W/m² 只有 1 行，确认是字段一致的真实 D+1 预报漏报而非损坏。另 41 行
`GHI=0,DHI>0` 全部位于晴空 GHI≤9.85 W/m² 的日出/日落低光区间，且总云量100%、
DNI=0、DHI仅1–6 W/m²，归因为区间平均与独立整数取整，不是字段错位。
total cloud 的 0/100 端点约为 20.76%/54.96%。20 个请求站对应
20 个不同返回点，本批 service-coordinate × time × lead 重复率为 0。完整证据见
