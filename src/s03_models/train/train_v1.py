# -*- coding: utf-8 -*-
"""Legacy v1 shared definitions retained while current adapters migrate.

The current fold-local adapters use the physical feature list and season
encoding here. Station/location/source-grid identity is not a model feature.
"""
import os
import sys
...
def tree_matrix(df):
    ...
    # Identity fields remain provenance only. No station/location/grid encoding.
    se_cat = pd.Categorical(df["season"], categories=list(SEASON_CODE.keys()))
    se = pd.get_dummies(se_cat, prefix="season").astype(float)
    se.index = df.index
    result = pd.concat([x, se], axis=1)
    assert_model_features(list(result.columns))
    return result
...
def tree_matrix_encoded(df, encoding="onehot", maps=None):
    ...
    - onehot：仅 season one-hot；
    - native：仅 season 保留为 pandas category；
    - target：仅 season 的训练子集目标均值编码。
    ...
    if encoding == "native":
        ...
        assert_model_features(list(x.columns))
        return x, ["season"], None
    if encoding == "target":
        if maps is None:
...
def target_maps(df, target_col):
    """在给定训练子集上只计算 season 的目标均值编码。"""
    return {"season": df.groupby("season")[target_col].mean().to_dict()}


def lin_pipe(alpha, kind="ridge"):
    """线性族管道：kind=ols（最小二乘）/ ridge（L2）/ huber（L1 的光滑代理）。"""
    assert_model_features(FEATURES_NUM + CAT_COLS)
